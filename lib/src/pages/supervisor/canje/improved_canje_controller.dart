...
class ImprovedCanjeController extends GetxController {
  ...
  ImprovedCanjeController(Usuario usuario) {
    this.usuario = usuario;
  }

  /// Registro de canje con validación de conexión obligatoria
  Future<void> registrarCanje(BuildContext context, Usuario usuario) async {
    ...
    Get.dialog(
      AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('No se puede procesar el canje sin conexión al servidor.'),
            SizedBox(height: 16),
            Container(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final hasConnection = await _verifyConnection();
              if (hasConnection) {
                registrarCanje(Get.context!, usuario!);
              } else {
                Get.snackbar(
                  'Sin Conexión',
  ...
  bool _validateFields() {
    // Verificar que tenga al menos valores a recibir y entregar
    final hasReceiveValues = [
    ...
    if (!hasReceiveValues) {
      Get.snackbar(
        'Campos Requeridos',
        'Debe ingresar al menos una denominación a recibir',
    ...
    if (!hasDeliveryValues) {
      Get.snackbar(
        'Campos Requeridos',
        'Debe ingresar al menos una denominación a entregar',
        backgroundColor: Colors.orange,
        colorText: Colors.white,
      );
      return false;
    ...
    await Get.dialog(
      AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('No se pudo procesar el canje:'),
            SizedBox(height: 8),
            Text(
