...
class TransaccionesController extends GetxController {
  ...
  TipoMovimientoProviderOffline tipoMovimientoProviderOffline =
      TipoMovimientoProviderOffline();
  ...
  void setFecha(bool isInicio, DateTime fecha) {
    if (isInicio) {
      fechaInicio.value =
          fecha.toString().substring(0, 10); // Formato YYYY-MM-DD
    } else {
  ...
  Future<List<Movimiento>> getMovimientos(String fechainicio, String fechafin,
      String idTipoMovimiento, String idPeaje) async {
    if (fechaInicio.isNotEmpty && fechaFin.isNotEmpty) {
      print(
          "Filtrando transacciones de ${fechaInicio.value} a ${fechaFin.value} - $idTipoMovimiento ");
      bandera.value = 1;
      return await movimientoProvider.findByDateTipoMovimiento(
          fechaInicio.value, fechaFin.value, idTipoMovimiento, idPeaje);
    } else {
      bandera.value = 0;
      return await movimientoProvider.findByTipoMovimiento(
          idTipoMovimiento, idPeaje);
    }
  }

  void openBottomSheet(
      BuildContext context, Movimiento movimiento, int bandera) async {
    // For liquidación (idTipoMovimiento == '4') we force bandera = 2 so faltantes/sobrantes are always enabled
    final int detalleBandera = movimiento.idTipoMovimiento == '4' ? 2 : bandera;
    List<Movimiento> movimientos = [];

    if (movimiento.idTipoMovimiento == '4') {
      movimientos.add(movimiento);
      var result = await movimientoProvider
          .getRetirosParciales(movimiento.idturno ?? '0');
      movimientos.addAll(result);

      showBarModalBottomSheet(
        context: context,
        builder: (context) => DetalleTransaccion(
            movimientos: movimientos, bandera: detalleBandera),
      );
    } else {
