...
class AsignacionController extends GetxController {
  ...
  Future<List<Movimiento?>> getMovimientoPorUsuario(String idTurno) async {
    ...
    if (await isConnectedToServer()) {
      movimientos = await movimientoProvider
  ...
  void getFaltante(String idTurno) async {
    if (await isConnectedToServer()) {
      clearMovimiento();
      var resultFaltante =
          await movimientoProvider.getMovimientoByTurno(idTurno);
      faltante = resultFaltante.firstWhere((m) => m.idTipoMovimiento == '4',
          orElse: () => Movimiento());
      update();
    } else {
      faltante = movimientoOffline.getMovimientoByTurno(idTurno).firstWhere(
  ...
  void goToLiquidaciones(Usuario usuario) async {
    ...
    if (await isConnectedToServer()) {
      movimientos = await movimientoProvider
          .getMovimientoByTurno(usuario.idTurno ?? '0'); //cambiar getApertura
    } else {
      movimientos =
  ...
  void goToRetiroApertura(Usuario usuario, Movimiento movimiento) async {
    if (!await isConnectedToServer()) {
      movimiento =
          await movimientoOffline.getApertura(movimiento.idturno ?? '');
    }
    Get.to(
      () => RetiroAperturaPage(
          usuario: usuario, movimiento: movimiento), // Página a la que navegas
      arguments: usuario, // Envía el objeto Usuario como argumento
    );
  }
  ...
  void goToReportes(Usuario usuario) async {
    ...
    if (await isConnectedToServer()) {
      movimientos = await movimientoProvider
          .getMovimientoByTurno(usuario.idTurno ?? ''); //cambiar getApertura
    } else {
      movimientos =
          movimientoOffline.getMovimientoByTurno(usuario.idTurno ?? '0');
    }
  ...
  Future<List<Usuario>> getUsuarios(String idEstado, String idPeaje) async {
    if (await isConnectedToServer()) {
      List<Usuario> response =
          await usuarioProvider.findByEstadoTurno(idEstado, idPeaje);
  ...
  void openBottomSheetLiquidacion(
      BuildContext context, String idturno, int bandera) async {
    List<Movimiento>? movimientos;
    ...
    showBarModalBottomSheet(
      context: context,
      builder: (context) =>
          DetalleTransaccion(movimientos: movimientos, bandera: bandera),
    );
  }
}
