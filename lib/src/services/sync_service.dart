...
class SyncService extends GetxController {
  ...
  static const int CONNECTION_TIMEOUT_SECONDS = 10; // Aumentado de 3 a 10 segundos
  ...
  void _startPeriodicSync() {
    _syncTimer = Timer.periodic(
      Duration(seconds: SYNC_INTERVAL_SECONDS), 
  ...
  Future<SyncResult> performFullSync() async {
    ...
    try {
      ...
      final totalSuccess = results.fold(0, (sum, result) => sum + result.successCount);
      final totalFailed = results.fold(0, (sum, result) => sum + result.failedCount);
  ...
  Future<SyncResult> _syncLiquidations() async {
    return _syncBox(
      ...
      (movimiento) => _movimientoProvider.updateLiquidacionCompleta(movimiento),
      'Liquidaciones'
    );
  }
  ...
  Future<SyncResult> _syncBox(
    ...
    try {
      ...
      for (var key in keys) {
        ...
        try {
          final response = await syncFunction(movimiento)
  ...
  Future<void> clearAllOfflineData() async {
    try {
      ...
      _updatePendingCount();
      Get.snackbar('Datos Limpiados', 'Todas las transacciones offline han sido eliminadas');
    } catch (e) {
      log('Error limpiando datos offline: $e');
      Get.snackbar('Error', 'No se pudieron limpiar los datos offline');
    }
  }
}
...
class SyncResult {
  ...
  SyncResult._(this.success, this.successCount, this.failedCount, this.error, this.status);
  ...
  factory SyncResult.alreadyInProgress() =>
      SyncResult._(false, 0, 0, 'Sincronización en curso', SyncStatus.inProgress);
}
