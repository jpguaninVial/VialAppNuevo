...
class ReporteLiquidacionController extends GetxController {
  ...
  Future<void> guardarPDFEnServidor(Uint8List pdfBytes) async {
    try {
      ...
      if (respuesta['exito']) {
        Get.snackbar('Éxito', 'PDF guardado exitosamente en el servidor',
            backgroundColor: Colors.green, colorText: Colors.white);
      } else {
        Get.snackbar(
            'Error', 'No se pudo guardar el PDF: ${respuesta['mensaje']}',
            backgroundColor: Colors.red, colorText: Colors.white);
      }
    } catch (e) {
      Get.snackbar('Error', 'Error al enviar el PDF: $e',
          backgroundColor: Colors.red, colorText: Colors.white);
    }
  }
}
